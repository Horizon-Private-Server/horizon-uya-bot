...
class ctf_flagchaser(ctf_main):
    ...
    def enter(self, msg:dict):
        ...
        if self.state_machine.game_state.player.team == 'blue':
            self.dest_point = self.state_machine.game_state.object_manager.red_flag.location
        else:
            self.dest_point = self.state_machine.game_state.object_manager.blue_flag.location
